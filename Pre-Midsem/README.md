...
The chromosome `.fa` files are too large for GitHub and are not included
in this repository. Download them separately:

**[Download data.zip (Google Drive)](https://drive.google.com/file/d/1JUuClufcE9jrF9ilLhNqedvsy1lxvzsT/view?usp=sharing)**

After cloning the repository and extracting `data.zip`, place the
`data/` directory inside `midsem/`. The expected layout is:
```
midsem/
...
Note the slightly unusual nesting: each chromosome's FASTA file and its
corresponding bins TSV live together inside a per-chromosome folder
(`data/chr1.fa/chr1.fa` and `data/chr1.fa/chr1_200bp_bins.tsv`). The
script relies on this layout.
...
- `Output_curves_for_REST_k5_m3/` — sample output curves for REST with
  `k=5`, `m=3`.
